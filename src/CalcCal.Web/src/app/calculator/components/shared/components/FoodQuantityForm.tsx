...
export default observer(function FoodQuantityForm() {
    ...
    const [submittedValue, setSubmitedValue] = useState<number>()

    const handleFormSubmit = (event: FormEvent<HTMLFormElement>) => {
        event.preventDefault()
        const quantity = (event.currentTarget.elements[0] as HTMLInputElement).value
        console.log(value)
    }

    const handleValueChange = (e: ChangeEvent<HTMLInputElement>) => {
        setIsInitialState(false)
        const inputValue = e.target.value.length <= value.length ?
